        ...
        if(lightType ==0)
            {
                ...
                diffuse = max(dot((transpose(inverse(view))*vec4(pNormal,1.0)).xyz,lightDir),0.0);
            }
            else
            {
                ...
                if(dotProd > 0.0)
                {
                    ...
                    vec3 planeIncidentLight = (view*vec4(lightPos-intersectionPoint(vWorldSpace,towardEye),0.0)).xyz;
